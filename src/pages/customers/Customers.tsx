...
export default function Customers() {
  ...
  const loadCustomers = async (term: string, append: boolean = false) => {
    ...
    try {
      if (term.trim().length >= 2) {
        const rows = await customerService.searchCustomers(user.tenantId, term, 20);
        setCustomers(rows);
        setCursor(null);
        setHasMore(false);
        return;
      }
      const result = await customerService.getCustomersPaginated(user.tenantId, 20, append ? cursor : null);
      const nextRows = result?.customers || [];
      setCustomers(current => append ? [...current, ...nextRows] : nextRows);
  ...
  useEffect(() => {
    if (!user?.tenantId) return;
    void loadCustomers(searchTerm, false);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-[10px] font-black text-text/30 uppercase tracking-[0.3em] mt-2">
              Loaded customer register: {customers.length} clients
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="p-6 border-border/50 bg-surface/50 backdrop-blur-md">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black text-text/40 uppercase tracking-widest">Loaded Clients</span>
              <Users className="h-5 w-5 text-text/20" />
            </div>
            <div className="mt-4 flex items-baseline gap-2">
              <span className="text-4xl font-black text-text">{stats.total}</span>
              <span className="text-[10px] font-bold text-text/20 uppercase">Visible</span>
            </div>
          </Card>
          ...
          <Card className="p-6 border-danger/20 bg-danger/5">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black text-danger uppercase tracking-widest">Loaded Outstanding</span>
        ...
        <Card className="overflow-hidden border-border bg-surface">
                    ...
                    >
                      ...
                      <td className="px-6 py-5 text-center">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[8px] font-black text-text/30 uppercase">Visible</span>
                        </div>
                      </td>
                      <td className="px-6 py-5 text-center">
                        <div className={cn(
          ...
          {filteredCustomers.length === 0 && (
            <EmptyState
              ...
              description="Loaded customer results matching the current filters will appear here."
          ...
          {!searchTerm && hasMore && filteredCustomers.length > 0 && (
