...
function buildPdf(order: ReorderOrder, storeName: string) {
  const pdf = new jsPDF();
  pdf.setFontSize(18); pdf.text('REORDER / PURCHASE REQUIREMENT', 14, 18);
  pdf.setFontSize(11); pdf.text(storeName, 14, 27); pdf.text(order.reorderNumber, 145, 27);
  pdf.text(`Supplier: ${order.supplierName}`, 14, 36);
  pdf.text(`Date: ${new Date().toLocaleDateString('en-IN')}`, 145, 36);
  pdf.line(14, 42, 196, 42);
  let y = 50;
  pdf.setFontSize(9); pdf.text('Product', 14, y); pdf.text('Current', 105, y); pdf.text('Min', 130, y); pdf.text('Order Qty', 155, y); pdf.text('Unit', 184, y);
  y += 6;
  order.lines.forEach((line, index) => {
    if (y > 275) { pdf.addPage(); y = 20; }
    pdf.text(`${index + 1}. ${line.productName}`.slice(0, 48), 14, y);
    pdf.text(String(line.currentStock), 108, y); pdf.text(String(line.minimumStock), 132, y);
    pdf.text(String(line.orderQuantity), 160, y); pdf.text(line.unit || 'Unit', 184, y);
    y += 7;
  });
  pdf.line(14, y, 196, y); y += 8;
  pdf.text(`Total order quantity: ${order.totalItems}`, 14, y);
  pdf.text(`Estimated value: ${formatCurrency(order.estimatedAmount)}`, 120, y);
  y += 10; pdf.setFontSize(8); pdf.text('Stock changes only after an actual purchase is posted. Quantity/rate subject to supplier confirmation.', 14, y);
  return pdf;
}
...
export default function ReorderManagement() {
    ...
    try {
      ...
      setQuantities(current => {
        ...
        rows.forEach(product => {
          if (next[product.id] === undefined) next[product.id] = suggestedReorderQuantity(product.stockQuantity, product.minimumStock, product.reorderTarget);
        });
        return next;
      });
    } finally {
  ...
  const draftLines: ReorderLine[] = lowProducts.filter(product => selected[product.id]).map(product => ({
    ...
    orderQuantity: quantities[product.id] || 0, lastPurchasePrice: product.purchasePrice,
  }));
  const selectedSupplier = suppliers.find(row => row.id === supplierId);

  const saveOrder = async () => {
    if (!user?.tenantId || !selectedSupplier) { showToast('Select a supplier first.', 'danger'); return; }
    setSaving(true);
    try {
      const result = await reorderService.save(user.tenantId, { requestId: crypto.randomUUID(), supplier: selectedSupplier, lines: draftLines, notes });
      const order: ReorderOrder = { id: result.id, tenantId: user.tenantId, requestId: '', reorderNumber: result.reorderNumber, supplierId, supplierName: selectedSupplier.name, supplierPhone: selectedSupplier.phone, supplierEmail: selectedSupplier.email, lines: draftLines, totalItems: draftLines.reduce((sum, line) => sum + line.orderQuantity, 0), estimatedAmount: draftLines.reduce((sum, line) => sum + line.orderQuantity * Number(line.lastPurchasePrice || 0), 0), status: 'draft', notes, createdBy: user.uid, createdAt: new Date(), updatedAt: new Date() };
      setCurrentOrder(order); await reloadOrders(); showToast(`${result.reorderNumber} saved. Stock is unchanged.`, 'success');
    } catch (error) { showToast(error instanceof Error ? error.message : 'Reorder could not be saved.', 'danger'); }
    finally { setSaving(false); }
  };

  const downloadPdf = (order: ReorderOrder) => buildPdf(order, settings?.storeName || 'PharmaFlow').save(`${order.reorderNumber}.pdf`);
  const message = (order: ReorderOrder) => `Reorder ${order.reorderNumber}\nSupplier: ${order.supplierName}\n${order.lines.map(line => `${line.productName}: ${line.orderQuantity} ${line.unit}`).join('\n')}\nPlease confirm availability and rates.`;
  const shareWhatsApp = (order: ReorderOrder) => { downloadPdf(order); window.open(`https://wa.me/${(order.supplierPhone || '').replace(/\D/g, '')}?text=${encodeURIComponent(message(order) + '\nPDF has been downloaded; please attach it to this chat.')}`, '_blank', 'noopener,noreferrer'); };
  const shareEmail = (order: ReorderOrder) => { downloadPdf(order); window.location.href = `mailto:${order.supplierEmail || ''}?subject=${encodeURIComponent(`Reorder ${order.reorderNumber}`)}&body=${encodeURIComponent(message(order) + '\n\nPDF has been downloaded; please attach it before sending.')}`; };

  return <div className="p-4 md:p-8 space-y-6">
    <div><h1 className="text-2xl font-black">Reorder Management</h1><p className="text-sm text-text/55">Minimum-stock based purchase requirements. Reorders do not change inventory.</p></div>
    <div className="grid gap-4 md:grid-cols-3"><div className="rounded-xl border bg-white p-4"><p className="text-xs text-text/50">Configured low items</p><p className="text-2xl font-black text-red-600">{lowProducts.length}</p></div><div className="rounded-xl border bg-white p-4"><p className="text-xs text-text/50">Selected products</p><p className="text-2xl font-black">{draftLines.length}</p></div><div className="rounded-xl border bg-white p-4"><p className="text-xs text-text/50">Suggested order units</p><p className="text-2xl font-black">{draftLines.reduce((sum, line) => sum + line.orderQuantity, 0)}</p></div></div>
    ...
    <section className="rounded-2xl border bg-white p-4"><div className="flex items-center justify-between"><h2 className="font-black">Saved Reorders</h2><button onClick={() => void reloadOrders()} aria-label="Refresh"><RefreshCw className="h-4 w-4" /></button></div>{orders.map(order => <div key={order.id} className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t pt-3"><div><p className="font-black text-primary">{order.reorderNumber}</p><p className="text-xs text-text/50">{order.supplierName} · {order.totalItems} units · {order.status}</p></div><div className="flex gap-2"><button onClick={() => downloadPdf(order)} className="rounded-lg border p-2" aria-label="Download PDF"><Download className="h-4 w-4" /></button><button onClick={() => shareWhatsApp(order)} className="rounded-lg border p-2" aria-label="Share WhatsApp"><MessageCircle className="h-4 w-4" /></button><button onClick={() => shareEmail(order)} className="rounded-lg border p-2" aria-label="Share email"><Mail className="h-4 w-4" /></button></div></div>)}</section>
  </div>;
}
