...
export default function Purchases() {
  ...
  const loadPurchases = async (term: string, append: boolean = false) => {
    ...
    try {
      if (searching) {
        const rows = await purchaseService.searchPurchases(user.tenantId, term, 25);
        setPurchases(rows);
        setCursor(null);
        setHasMore(false);
        return;
      ...
      const result = await purchaseService.getPurchasesPage(user.tenantId, 25, append ? cursor : null);
      setPurchases(current => (append ? [...current, ...result.purchases] : result.purchases));
      ...
      showToast('Failed to load purchase history', 'danger');
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    if (!user?.tenantId) return;
    void loadPurchases(searchTerm, false);
      ...
      <div className="p-4 md:p-8 max-w-[1600px] mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-6">
          <Card className="p-4 md:p-6 flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-text/60">Loaded Purchase Entries</p>
          ...
          <Card className="p-4 md:p-6 flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-text/60">Loaded Purchase Value</p>
          ...
          <Card className="p-4 md:p-6 flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-text/60">Loaded Suppliers</p>
        ...
        {loadError ? (
          <Card className="p-6 md:p-10 text-center border border-danger/20 rounded-2xl">
            ...
            <Button className="mt-4 min-h-11" onClick={() => void loadPurchases(searchTerm, false)}>Retry</Button>
          </Card>
        ) : purchases.length === 0 ? (
          <EmptyState
            ...
            <Card className="hidden md:block overflow-hidden border border-border rounded-3xl bg-surface">
              ...
              {!searchTerm && hasMore && (
                <div className="border-t border-border p-4">
                  ...
                  >
                    {loadingMore ? 'Loading more…' : 'Load More Purchases'}
                  </Button>
                </div>
              )}
            </Card>
